...
type (
	...
	getEntriesCtrl chan []*journal.Event
)

type MemJournal struct {
	journal.EventTypeFactory

	entries   []*journal.Event
	index     map[string]map[string][]*journal.Event
	observers []observer

	incomingCh chan *journal.Event
...
type observer struct {
	accept map[journal.EventType]struct{}
	ch     chan *journal.Event
}

func (o *observer) dispatch(entry *journal.Event) {
	if o.accept == nil {
		o.ch <- entry
	}
...
func NewMemoryJournal(lc fx.Lifecycle, disabled journal.DisabledEvents) *MemJournal {
	m := &MemJournal{
		EventTypeFactory: journal.NewEventTypeFactory(disabled),

		index:      make(map[string]map[string][]*journal.Event, 16),
		observers:  make([]observer, 0, 16),
		incomingCh: make(chan *journal.Event, 256),
...
func (m *MemJournal) RecordEvent(evtType journal.EventType, obj interface{}) {
	if !evtType.Enabled() {
...
// events types.
func (m *MemJournal) Observe(ctx context.Context, replay bool, include ...journal.EventType) <-chan *journal.Event {
	var acc map[journal.EventType]struct{}
	if include != nil {
		acc = make(map[journal.EventType]struct{}, len(include))
...
func (m *MemJournal) Entries() []*journal.Event {
...
func (m *MemJournal) process() {
	processCtrlMsg := func(message interface{}) {
		...
		case getEntriesCtrl:
			cpy := make([]*journal.Event, len(m.entries))
			copy(cpy, m.entries)
			msg <- cpy
			close(msg)
		}
	...
	processClose := func() {
		m.entries = nil
		m.index = make(map[string]map[string][]*journal.Event, 16)
	...
	for {
		...
		case entry := <-m.incomingCh:
			...
			if events == nil {
				events = make(map[string][]*journal.Event, 16)
