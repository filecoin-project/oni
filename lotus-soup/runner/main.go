...
func main() {
	...
	outputDir := flag.String("output", "", "path to output directory (will use temp dir if unset)")
	flag.Parse()
	...
	for j := 1; j <= *runs; j++ {
		...
		jobs <- jobDefinition{runNumber: j, compositionPath: compFile, outputDir: dir, skipStdout: skipStdout}
	}
	close(jobs)
