...
func defConfForType(t repo.RepoType) interface{} {
	switch t {
	case repo.FullNode:
...
type MemRepo struct {
	...
	configF func(t repo.RepoType) interface{}
...
type MemRepoOptions struct {
	...
	ConfigF  func(repo.RepoType) interface{}
	KeyStore map[string]types.KeyInfo
}
...
func (mem *MemRepo) APIToken() ([]byte, error) {
	...
	defer mem.api.Unlock()
	if mem.api.ma == nil {
		return nil, repo.ErrNoAPIToken
	}
	return mem.api.token, nil
}

func (mem *MemRepo) Lock(t repo.RepoType) (repo.LockedRepo, error) {
	select {
	case mem.repoLock <- struct{}{}:
	default:
		return nil, repo.ErrRepoAlreadyLocked
	}
	mem.token = new(byte)
